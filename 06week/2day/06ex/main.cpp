...
int main() {
	...
	for (int i=0; i<20; i++) {
      ...
      // In each catch block write out what the exception's type was. And what the exceptions value was.
		  ...
		  cout << "int: " << intError << endl;
		} catch (double doubleError) {
		  cout << "double: " << doubleError << endl;
		} catch (const char* stringError) {
		  cout << "string: " << stringError << endl;
		} catch (bool boolError) {
      cout << "bool: " << boolalpha << boolError << endl;
    } catch (float floatError) {
      cout << "float: " << floatError << endl;
    } catch (...) {
      cout << "elseError" << endl;
    }
  }
